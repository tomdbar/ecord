...
if __name__ == "__main__":
    ...
    training_config = DQNTrainingConfig(
        # Episode
        graph_generator = ErdosRenyiGraphGenerator([40, 40], 0.15, EdgeType.SIGNED),
        tradj_per_graph = 1,
        num_steps_per_rollout = -1,
        num_parallel_graphs = 8,
        intermediate_reward = 1/40,
        revisit_reward = None,
        ...
        lr = 3e-4,
        gamma = 0.7,
        init_bce_weight=0.01, # Only matters if network_config.use_network_initialisation is True.
    ...
    test_graph_configs = [
        ...
        TestGraphsConfig.from_file(
            label='ER100',
            graph_loc='_graphs_eco_dqn/testing/ER_200spin_p15_50graphs',
            num_load=50,
            **test_config_args,
        ),
        TestGraphsConfig.from_file(
    ...
    trainer = Trainer(
        ...
        use_network_initialisation=training_config.use_network_initialisation,
    ...
    fig.savefig(
        os.path.join(trainer.save_loc, "scores_zoom.png"),
        bbox_inches='tight'
    )